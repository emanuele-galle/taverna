      ...
      <div className="flex justify-center gap-2 mb-12">
        {categories.map((cat) => (
          <button
            ...
            className={`px-6 py-2.5 rounded-full font-sc text-base tracking-wider transition-all duration-300 ${
              activeCategory === cat
        ...
        {filteredImages.map((image, index) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-end p-4">
              <div>
                <span className="font-sc text-xs tracking-wider text-gold/90 block mb-0.5">{image.category}</span>
